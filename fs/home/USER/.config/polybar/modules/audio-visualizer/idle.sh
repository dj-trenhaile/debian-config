...
do
    ...
    if [ $crest_idx -ge $((top_idx + BARS_RANGE)) ]
    then
        crest_idx=$wave_start_idx
    fi
...
then
    ...
    while [ $low_idx -le $top_idx ] && [ $crest_value -gt 0 ]
    do
        ...
        if [ $new_crest_idx -lt $top_idx ]
        then
            if [ $high_idx -gt $top_idx ]
            then
                high_idx=$top_idx
            fi
